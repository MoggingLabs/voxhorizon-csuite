...
pass; gitleaks, dependabot, and CodeQL active; `WORKER_SHARED_SECRET` rotated on
production and absent here; the static isolation guard passes.

Issues: M0-1 repo + conventions; M0-2 docs skeleton; M0-3 the eight ADRs; M0-4
issue + PR templates (incl. security); M0-5 `ci.yml`; M0-6 CodeQL + gitleaks +
dependency-review + dependabot; M0-7 rotate prod `WORKER_SHARED_SECRET` (first);
M0-8 branch protection.
